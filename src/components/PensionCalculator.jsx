...
export default function PensionCalculator() {
 ...
 const [customTickerInput, setCustomTickerInput] = useState('');
  ...
  // AI Portfolio Optimization State
  const [isAiOptimizing, setIsAiOptimizing] = useState(false);
  const [aiThesisData, setAiThesisData] = useState(null);
 ...
 console.error('Error checking auth:', e);
  ...
  // 2b. AI-Driven Stock Selection & Knapsack Lot Optimization
  const handleAiOptimize = async () => {
    setIsAiOptimizing(true);
    try {
      const res = await fetch('/api/pension/ai-generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          riskProfile,
      ...
      if (res.ok) {
        ...
        if (data.presetStocks && data.presetStocks.length > 0) {
          ...
          // Set the exact discrete optimal lots calculated by AI Knapsack Optimizer
          if (data.optimalLots) {
            setManualLots(data.optimalLots);
          }

          // Store AI thesis and advice
          setAiThesisData({
            thesis: data.portfolioThesis,
          ...
          // Sync custom preset to DB & localStorage
          syncCustomPresetToDB(data.presetStocks);
        ...
        showToast(errData.error || 'Gagal mengoptimasi dengan AI', 'error');
      }
    } catch (err) {
      console.error('AI Pension Optimize Error:', err);
