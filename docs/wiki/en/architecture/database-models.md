...
Manages the offline Llama.cpp queue and records AI-generated fundamental analyses:
* `AiStockResearch`: Stores the final output (fundamental, valuation, trend, recommendation, and numerical `score` 0-100) generated by the AI agent. Kept to a limit of 1 per quarter.
* `AiResearchQueue`: The async job queue polling table (status: `PENDING`, `PROCESSING`, `COMPLETED`, `FAILED`).
* `AiAuditLog`: Full audit trail of every request processed by the LLM, recording `modelName`, input tokens (`promptTokens`), output tokens (`completionTokens`), total tokens (`totalTokens`), duration (`latencyMs` and `durationSec`), generation speed (`tokensPerSec`), prompt text, and raw response.
